...
function createTaskCard(t, isCompleted, priority) {
    ...
    editPanel.innerHTML = `
        <div class="edit-row">
            <input class="edit-input" type="text" value="${(t.task_name || t.title || '').replace(/"/g, '&quot;')}" />
            <button class="edit-btn edit-save">保存</button>
            <button class="edit-btn edit-cancel">✕</button>
        </div>`;
    ...
    editPanel.querySelector('.edit-save').addEventListener('click', async (e) => {
        ...
        if (newTitle && newTitle !== (t.task_name || t.title)) {
            await action("update_title", t.id, { task_name: newTitle });
        }
        wrap.classList.remove('expanded');
    });
    editPanel.querySelector('.edit-cancel').addEventListener('click', (e) => {
        e.stopPropagation();
        editInput.value = t.task_name || t.title || '';
        wrap.classList.remove('expanded');
    });
    // パネル内クリックの伝播を止める
    ...
    sl.addEventListener('click', (e) => {
        ...
        // スワイプ操作で開いている場合は閉じるだけ（swipe.jsで処理済み）
