...
class SwipeableRow {
    ...
    _updateButtonExpansion() {
        ...
        if (dx > 0 && this._leftButtons.length >= 1) {
            ...
            if (dx <= halfCard || others.length === 0) {
                ...
                // 50%超: 完了が通知を押しつぶす
                const overRatio = Math.min(1, (dx - halfCard) / (halfCard * 0.4));
            ...
            if (absDx <= halfCard || others.length === 0) {
                ...
                const overRatio = Math.min(1, (absDx - halfCard) / (halfCard * 0.4));
    ...
    _flyOff(targetX, actionType) {
        ...
        const startCollapse = () => {
            ...
            this.wrap.style.transition = 'height 250ms ease-out, opacity 150ms ease-out';
        ...
        setTimeout(startCollapse, 300); // fallback
    }
