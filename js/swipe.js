...
class SwipeableRow {
    ...
    _updateButtonExpansion() {
        ...
        if (dx > 0 && this._leftButtons.length >= 2) {
            // 右ドラッグ → 左ボタン群が見える
            const primary = this._leftButtons[0]; // 完了
            const others = this._leftButtons.slice(1); // 通知 etc
            const expandStart = this.leftWidth;
            const expandEnd = this.leftWidth * 2;

            if (dx > expandStart) {
                const ratio = Math.min(1, (dx - expandStart) / (expandEnd - expandStart));
                primary.style.flex = '1';
                primary.style.minWidth = `${dx * 0.9}px`;
                others.forEach(btn => {
                    btn.style.maxWidth = `${Math.max(0, 60 * (1 - ratio))}px`;
                    btn.style.opacity = String(1 - ratio);
                    btn.style.overflow = 'hidden';
                    btn.style.padding = ratio > 0.7 ? '0' : '';
                });

                // フルスワイプ圏内の視覚フィードバック
                if (dx > window.innerWidth * SWIPE_CONFIG.FULL_SWIPE_RATIO) {
        ...
        } else if (dx < 0 && this._rightButtons.length >= 2) {
            // 左ドラッグ → 右ボタン群が見える
            const absDx = Math.abs(dx);
            const primary = this._rightButtons[this._rightButtons.length - 1]; // 削除（最後）
            const others = this._rightButtons.slice(0, -1); // 詳細, 優先 etc
            const expandStart = this.rightWidth;
            const expandEnd = this.rightWidth * 2;

            if (absDx > expandStart) {
                const ratio = Math.min(1, (absDx - expandStart) / (expandEnd - expandStart));
                primary.style.flex = '1';
                primary.style.minWidth = `${absDx * 0.9}px`;
                others.forEach(btn => {
                    btn.style.maxWidth = `${Math.max(0, 60 * (1 - ratio))}px`;
                    btn.style.opacity = String(1 - ratio);
                    btn.style.overflow = 'hidden';
                ...
                if (absDx > window.innerWidth * SWIPE_CONFIG.FULL_SWIPE_RATIO) {
                    this.wrap.classList.add('swipe-commit');
                } else {
                    this.wrap.classList.remove('swipe-commit');
                }
    ...
    _determineSnapTarget(velocity) {
        ...
        // フルスワイプ判定（画面幅ベース）
        const fullSwipeThreshold = screenW * SWIPE_CONFIG.FULL_SWIPE_RATIO;
